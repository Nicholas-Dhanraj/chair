...
export default function Yellow({ ...props }) {
  const group = useRef();
  const { nodes, materials } = useGLTF("/yellow-chair/scene.gltf");
  return (
      ...
      <group rotation={[-Math.PI / 2, 0, 0]}>
        <group position={[0, 0, 0.11]} rotation={[Math.PI / 2, 0, 0]}>
          <group position={[0.06, 27.41, -0.65]}>
            <mesh
              geometry={nodes.PEARL_GREY.geometry}
              material={materials.PEARL_GREY}
            />
            <mesh
              geometry={nodes.RAINBOW_BUTTONS.geometry}
...
useGLTF.preload("/yellow-chair/scene.gltf");
